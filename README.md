...
1. 우선 다운받은 원격에서 addonLibrary를 압축을 프로젝트 경로에 푼다
...
5. 링커 -> 입력 -> 추가 종속성에서 SDL2.lib와 SDL2main.lib를 추가한다. (이외 추가 라이브러리가 있다면 똑같이 추가한다.)
   5.1 만약에 설정하고 실행이 되지 않는다면 (shell32.lib를 추가한다)
   5.2 SDL2.dll파일은 소스파일 있는 경로에 옮기거나 복사하면 된다
